...
class Metrics:
    def __init__(self, model, test_data, result_folder, best_trial_check, channel):
        self.model = model
        self.device = DEVICE
    ...
    def compute_metrics(self, method_name, method, method_map, inputs, labels):
        ...
        if method_name == "Occlusion":
            sensitivity_score = sensitivity_max(method.attribute, inputs, target=labels, sliding_window_shapes=(1, 3, 3), strides=(1, 2, 2))
        else:
